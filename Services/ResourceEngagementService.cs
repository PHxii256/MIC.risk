...
public class ResourceEngagementService : IResourceEngagementService
{
    ...
    public async Task<IEnumerable<ResourceEngagementStatsDto>> GetResourceStatsAsync(
        CancellationToken cancellationToken = default)
    {
        var activeEmployeeCount = await _context.Employees.CountAsync(e => e.Active, cancellationToken);
        ...
        {
            var resourceEngagements = engagements.Where(e => e.ResourceId == resource.Id).ToList();
            ...
            var completionRate = activeEmployeeCount == 0
                ? 0
                : resource.Type.Equals("Quiz", StringComparison.OrdinalIgnoreCase)
                    ? (double)quizCompletionCount / activeEmployeeCount * 100
                    : (double)viewCount / activeEmployeeCount * 100;
