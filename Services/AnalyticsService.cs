...
{
    private const int CriticalRiskScoreThreshold = 15;
    private const int WeakControlsThreshold = 2;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var earlyWarning = new EarlyWarningIndicatorsDto(
            CriticalResidualRisks: openReports.Count(r => GetEffectiveRiskScore(r) >= CriticalRiskScoreThreshold),
            WeakControls: openReports.Count(r => GetEffectiveMeasuresEffectiveness(r) <= WeakControlsThreshold),
        ...
        var matrixCells = openReports
            ...
            .Select(g => new ResidualRiskMatrixCellDto(g.Key.Severity, g.Key.Frequency, g.Count()))
        ...
        return new AnalyticsDashboardDto(
            ...
            new ResidualRiskMatrixDto(matrixCells)
        );
    }
    ...
    private static int GetEffectiveRiskScore(RiskReport report) =>
        GetEffectiveEvaluation(report).RiskScore;

    private static int GetEffectiveMeasuresEffectiveness(RiskReport report) =>
        GetEffectiveEvaluation(report).MeasuresEffectiveness;
}
